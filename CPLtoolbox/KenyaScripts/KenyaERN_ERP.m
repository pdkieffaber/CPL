...
for sub=1:length(filelist)
    ...
    %Bandpass filter
    EEG  = pop_basicfilter( EEG,  1:34 , 'Boundary', 'boundary', 'Cutoff', [filterparams(1) filterparams(2)], 'Design', 'butter', 'Filter', 'bandpass', 'Order',  4 ); % GUI: 21-Nov-2022 19:08:55
    ...
    [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
    EEG = pop_epochbin( EEG , [-1000.0  1000.0],  [-500 -300]); % GUI: 21-Nov-2022 19:12:05
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'overwrite','on','gui','off'); 

    %Flag artifacts in EEG channels
    %Absolute threshold
    EEG  = pop_artextval( EEG , 'Channel',  3:34, 'Flag',  1, 'LowPass',  -1, 'Threshold', [ -100 100], 'Twindow', [-500 600] ); % GUI: 21-Nov-2022 19:13:50
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'overwrite','on','gui','off'); 
    
    %Window max-min threshold
    EEG  = pop_artmwppth( EEG , 'Channel',  3:34, 'Flag',  2, 'LowPass',  -1, 'Threshold',  100, 'Twindow', [-500 600], 'Windowsize',  200,...
    'Windowstep',  100 ); 
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'overwrite','on','gui','off'); 

    %Blocking and Flatline
    EEG  = pop_artflatline( EEG , 'Channel',  3:34, 'Duration',  700, 'Flag',  3, 'LowPass',  -1, 'Threshold', [ -.01 .01], 'Twindow',...
    [ min(EEG.times) max(EEG.times)] );
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'overwrite','on','gui','off'); 
