...
for sub=1:length(filelist)
    ...
    for i=1:length(EEG.event)
        switch EEG.event(i).type
            case 'S  5' %Recode Standard Events
                temp(i).type=10;
                temp(i).code=10;
            case 'S  8' %Recode Deviant Events
                temp(i).type=20;
                temp(i).code=20;
        end
    end
    ...
    %Bandpass filter
    EEG  = pop_basicfilter( EEG,  1:34 , 'Boundary', 'boundary', 'Cutoff', [ 0.2 20], 'Design', 'butter', 'Filter', 'bandpass', 'Order',  4 ); % GUI: 21-Nov-2022 19:08:55
    ...
    [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
    EEG = pop_epochbin( EEG , [-200.0  800.0],  'pre'); % GUI: 21-Nov-2022 19:12:05
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'overwrite','on','gui','off'); 

    %Flag artifacts in EEG channels
    EEG  = pop_artextval( EEG , 'Channel',  3:34, 'Flag',  1, 'LowPass',  -1, 'Threshold', [ -100 100], 'Twindow', [ -200 799] ); % GUI: 21-Nov-2022 19:13:50
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'overwrite','on','gui','off'); 
